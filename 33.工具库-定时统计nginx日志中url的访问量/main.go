...
var (
	listArray []numMap
	mu sync.Mutex
	statisticsUrl map[string]int
	urlIndex = flag.Int("i", 5, "url index")
	cronTime = flag.String("t", "*/5 * * * * ?", "cron Time")
	filePath = flag.String("p", "./nginx.log", "nginx log path")
)
	...
	go func() {
		...
		if err := c.AddFunc(*cronTime, func() {
			...
			for k, v := range statisticsUrl {
				listArray = append(listArray, numMap{
					key: k,
					num: v,
				})
			}
